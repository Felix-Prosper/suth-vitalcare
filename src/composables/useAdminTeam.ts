...
export const useAdminTeam = () => {
  ...
  const deleteTeam = async (team: Team, silent = false) => {
    if (!silent) {
      const ok = await showConfirm(`คุณต้องการลบทีม "${team.name}" ใช่หรือไม่? กิจกรรมที่เกี่ยวข้องกับทีมนี้อาจได้รับผลกระทบ`, undefined, 'ยืนยันลบ', 'warning', true);
      if (!ok) return;
    }
  ...
  const kickMember = async (teamId: number, userId: number, silent = false) => {
    if (!silent) {
      const ok = await showConfirm('คุณต้องการคัดสมาชิกคนนี้ออกจากทีมใช่หรือไม่?');
      if (!ok) return false;
    }
